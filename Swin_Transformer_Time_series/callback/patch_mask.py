...
class PatchCB(Callback):
    ...
    def set_patch(self):
        ...
        """
        #xb_patch = create_patch(self.xb, self.patch_len, self.stride)    # xb: [bs x seq_len x n_vars]
...
#    """
#    if len(xb.shape) < 2:
#        raise ValueError(f"Expected xb to have at least 2 dimensions, but got shape {xb.shape}")
#    seq_len = xb.shape[1]
    
#    # Check if patch_len is greater than seq_len
#    if patch_len > seq_len:
#        patch_len = seq_len  # Adjust patch_len to seq_len or take other appropriate action
#    #seq_len = xb.shape[1]
